...
class FinanceController extends HomeController {
	protected $config = array(
		'app_type'	=> 'common',
		'admin'		=> 'account_list,add_account,save_account,edit_account,del_account,readtopic',
		'write'		=> 'add_income,add_payment,add_transfer,save_transfer,finance_add,get_topic,add_topic,readtopic',
		'read'		=> 'read_account,finance_list,get_topic,add_topic,readtopic');
	...
	public function save(){
		...
		$model = M('topic');
		if(!empty($_POST['id'])){
			$topic['user_id'] = get_user_id($_POST['agent']);
			$topic['user_name'] = get_user_name_empno($_POST['agent']);
			$topic['emp_no'] = $_POST['agent'];
			$topic['number'] = $_POST['number'];
			$topic['name'] = $_POST['name'];
			$result = $model->where('id='.$_POST['id'])->save($topic);
		}else{
			$topic['user_id'] = get_user_id($_POST['agent']);
			$topic['user_name'] = get_user_name_empno($_POST['agent']);
			$topic['emp_no'] = $_POST['agent'];
			$topic['number'] = $_POST['number'];
			$topic['name'] = $_POST['name'];
	    	$result = $model->add($topic);
		}
		

    	$this->success('操作成功！',U('Finance/get_topic'));
