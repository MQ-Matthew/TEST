...
class FormController extends HomeController {
	protected $config = array('app_type' => 'public','admin' => 'del,move_to,folder_manage,field_type,field_manage,getform,getketi');
	...
	private function _folder_export($model, $map) {
		...
		//编号，类型，标题，登录时间，部门，登录人，状态，审批，执行，抄送，审批情况，自定义字段
	...
	public function getform($type){
		...
		if($type == '课题经费'){
	        ...
	        foreach ($list as $k => $v) {
	            if (!empty($v['name'])) {
	            	$udf = json_decode($v['udf_data'],true);
	                $result.="<option value=" . $udf[343].'-'.$v['name'] . ">" . $udf[343]."&nbsp;".$v['name'] . "</option>";
	            }
	        }
		}else if ($type == '项目经费') {
			$map['folder'] = 2;
	...
	public function getketi($keti){
		$name = substr($keti,strpos($keti,'-')+1);
		$map['folder'] = 1;
		$map['name'] = $name;
		$list = M('Form')->where($map)->getField('udf_data');
		$udf = json_decode($list,true);
		echo $udf[345];
