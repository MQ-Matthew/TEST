...
class PopupController extends HomeController {
	...
	function read($id) {
		...
		switch ($type) {
			...
			case "position" :
                ...
                $where['identity'] = array('like', '%_'.$id.'|%');   //2018-1-25查出用户多个职位
				$where['is_del'] = array('eq', 0);
				$data = $model -> where($where) -> select();
				break;
    ...
    //财务科
    private function _dept_caiwu_list() {
        $model = M("Dept");
        $list = array();
        $where['is_del'] = 0;
        $where['id'] = 11;
    ...
    //人事科
    private function _dept_renshi_list() {
        $model = M("Dept");
        $list = array();
        $where['is_del'] = 0;
        $where['id'] = 17;
    ...
    //合同管理员列表
    private function _position_hetong_list() {
        $model = M("Position");
        $list = array();
        $where['id'] = 6;
    ...
    function refer_caiwu() {
        
        $plugin['jquery-ui'] = true;
        $this -> assign("plugin", $plugin);

        $this -> _dept_caiwu_list();
        $this -> _position_list();
        //$this -> _position_hetong_list();
        $this -> _group_list();
        $this -> assign('type', 'dept');
    ...
    function refer_renshi() {
        
        $plugin['jquery-ui'] = true;
        $this -> assign("plugin", $plugin);

        $this -> _dept_renshi_list();
        $this -> _position_list();
        //$this -> _position_hetong_list();
        $this -> _group_list();
        $this -> assign('type', 'dept');
    ...
    function identity() {
        ...
        $model = M("User");
        $map['id'] = I('id');
        $list = $model -> field('id,name,identity') ->where($map)-> find();
	...
	function real_img(){
		if (!$image = $_POST["img"]) {
			...
			$real_img = substr($image,4);;
			$result['result_code'] = 1;
			$result['result_des'] = $real_img;
...
///echo "<hr>";
   ///     dump(popup_tree_menu($tree));
    ...
    public function flow_project(){
        ...
        //导入ztree
        $ztree = D('Project')->getTree(0,'id,name,sort,pid');

       /// dump($ztree);die;

        $JsonTree = json_encode($ztree);

        ///dump($JsonTree);die;
        $this->assign("zNodes", $JsonTree);
    ...
    public function sendmessage($info=''){
        //dump($info);
        $map['emp_no'] = $info['emp_no'];
        $mobile_tel = M('User')->where($map)->getField('mobile_tel');
        
        $info['mobile'] = $mobile_tel;
        $this->assign("info", $info);
        $this -> display();

    }
    //通用发送短信方法
    public function send_message($emp_no='',$message=''){
        //dump($emp_no);die();
        $array_emp = explode("|", $emp_no);        
        $map['emp_no'] = array('in', $array_emp);
        $rs = M('User')->where($map)->select();
        //dump($rs); die();
        $info = array();
        foreach ($rs as $key => $value) {
        	$info['user_name'] = $value['name'].",".$info['user_name'];
        	$info['mobile'] = $value['mobile_tel'].",".$info['mobile'];
        	$info['emp_no'] = $value['emp_no'].",".$info['emp_no'];
        }     
    ...
    */
    function send_sms($data, $emp_no_list) {
    	//dump($emp_no_list);die();        
        if (strstr($emp_no_list, ',')) {
        ...
        if (!$conn)
        {
            ...
            //$this -> success('Could not connect: ' . mysql_error());
            //die();
        }
       // mysql_query("set names utf8");
        // 从表中提取信息的sql语句
        
        $content = iconv("utf-8","gb2312//IGNORE",$data);
		//$content = $data;
		//dump($sms_user_list);die();
        $strsql="INSERT INTO api_mt_caigou (MOBILES,CONTENT) VALUES ('".$sms_user_list."','".$content."')";
        //$strsql="INSERT INTO api_mt_caigou (MOBILES,CONTENT) VALUES ('13001250716','is test!')";
		// 执行sql查询
