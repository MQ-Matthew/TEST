...
class  FlowModel extends CommonModel {
    ...
    */
	...
	public function back_to($flow_id, $back_emp_no,$back_step) {

		$model = D("FlowLog");
		$where['flow_id'] = array('eq', $flow_id);
		$where['emp_no'] = array('eq', $back_emp_no);
		$where['is_del'] = array('eq', 0);
		$where['step'] = array('eq', $back_step);
		$logdata = D("FlowLog") -> where($where) -> getField('step');
		if (empty($logdata)) {
			// $data1['step'] = 19;
			// $model1 = M('Flow');
...
/*
	...
	function is_last_confirm($flow_id) {
		...
		$num = substr_count($confirm,'|');
		
		$log_where['result'] = array('neq', 2);
		$log_where['is_del'] = array('eq', 0);
		$log_where['flow_id'] = array('eq', $flow_id);
		$logs = M("Flow_log") -> where($log_where) -> Field("step") -> select();
		$logs_num = count($logs)-1;
		$confirm_list = array_filter(explode("|", $confirm));
