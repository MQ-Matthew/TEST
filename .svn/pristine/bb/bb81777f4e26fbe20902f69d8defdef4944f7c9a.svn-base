...
class  GroupModel extends CommonModel {
	...
	function save_user($user_list, $group_list) {
		...
		$sql = 'INSERT INTO ' . $this -> tablePrefix . 'group_user (user_id,emp_no,user_name,dept_id,position_id,sort,group_id,amount) ';
		//$sql .= ' SELECT a.id, b.id FROM ' . $this -> tablePrefix . 'user a, ' . $this -> tablePrefix . 'group b WHERE ' . $where;
		$sql .= ' SELECT a.id,a.emp_no,a.name,a.dept_id,a.position_id,a.sort,b.id,b.amount FROM ' . $this -> tablePrefix . 'user a, ' . $this -> tablePrefix . 'group b WHERE ' . $where;
		//dump($sql);
