...
class FlowTypeController extends HomeController {
	...
	function flow_step($flow_type_id) {
		$flow_type=M('FlowType')->find($flow_type_id);
		...
		if (IS_POST){
			...
			if ($opmode == "del") {
				$this -> _destory($model -> id,'FlowStep');
			}
		}

		$model = D("FlowStep");
		$where['flow_type_id'] = array('eq', $flow_type_id);
	...
	public function condition($step_id) {
		if (IS_POST) {

			$data['id'] = I('step_id');
			$data['condition_name'] = I('condition_name');
			$request = $_REQUEST;

			unset($request['flow_type_id']);
			unset($request['condition_name']);
			unset($request['ajax']);

			$data['condition'] = json_encode($request, true);
	 
			$list = M('FlowStep') -> save($data);
		...
		$flow_step=M('FlowStep')->find($step_id);
		
		$where_field['controller'] = 'Flow';
		$where_field['row_type'] = $flow_step['flow_type_id'];
		$udf_field = M('UdfField') -> where($where_field) -> getField('id,name');
