...
class HomeController extends Controller {
	...
	protected function _assign_menu() {
        ...
        if(empty($top_menu)){$top_menu='87';}//默认审批
		if (!empty($top_menu)) {
	...
	protected function _search($model = null) {
		...
		// $fields1 =get_model_fields(M('user_file'));
		...
		foreach ($request as $val) {
			...
			if (in_array($field, $fields)) {
				...
				if ($prefix == "in_") {
					$map[$field] = array('between', trim($_REQUEST[$val]));
					//var_dump($map[$field]);die;
				}
			}
		}
		//print_r($map);die;
	...
	protected function _lists($model, $map, $sort = '') {
		//排序字段 默认为主键名
		if (isset($_REQUEST['_sort'])) {
			$sort = $_REQUEST['_sort'];
		} else if (in_array('sort', get_model_fields($model))) {
		...
		$count_model = clone $model;
		//取得满足条件的记录数
		$count = $count_model -> join('oa_user_file  ON oa_user.id = oa_user_file.user_id') -> where($map) -> count();
		...
		if ($count > 0) {
			...
			$p = new \Page($count, $list_rows);
			//分页查询数据
			$vo_list = $model -> join('oa_user_file  ON oa_user.id = oa_user_file.user_id') -> where($map) -> order($sort) -> limit($p -> firstRow . ',' . $p -> listRows) -> select();
    ...
    //********************************************
    //SSO单点登陆 By Dokey  2017-3-21
    //检测Token是否存在，判断后自动登录
    //********************************************
    protected function sso_login(){
        //获取SSO传递过来的 Token
        if(!empty( $_GET['Token']))
        {
            $Token =  $_GET['Token'];//Token字符串
            $mToken = base64_decode($Token);//还原编码
            $CheckDate = explode(',',$mToken); //时间串+用户名
            date_default_timezone_set('PRC');//调整时区
            $datetime = $CheckDate[0];//时间戳
            $emp_no = $CheckDate[1]; //用户名

            if($datetime ==date("Y-m-d H:00:00"))//检测时间戳是否一致
            {
                $map = array();
                $map['emp_no'] = $emp_no;
                ...
                if (false == $auth_info) {
                    ...
                    //header('Location: ' . U("Flow/folder&fid=confirm"));                         
                }
            }
        }
