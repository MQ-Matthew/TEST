...
class PublicController extends Controller {
	...
	public function check_login() {		
		...
		$map['password'] = array('eq', md5(I('password')));
		...
		if (false == $auth_info) {             
			...
			$data['dept_id'] = $auth_info['dept_id'];
			$data['dept_name'] = get_dept_name($auth_info['dept_id']);

			$data['user_pic'] = $auth_info['pic'];

			$data['_u'] = think_encrypt($auth_info['id']);
			$time = time();
			$data['_t'] = think_encrypt($time);
			$data['_k'] = think_encrypt(make_token($auth_info['id'], $time));
